...
export default class Outpayments {
  ...
  public transfer({
    ...
    return validateTransfer(payoutRequest).then(() => {
      ...
      return this.client
        .post<void>("/outpayments", payoutRequest)
        .then(() => referenceId);
    });
  }
   ...
   * of `MtnMoMoError`. Check [`src/error.ts`](https://github.com/rileydigitalservices/payhere-node-sdk/blob/master/src/errors.ts)
  ...
  public getTransaction(referenceId: string): Promise<Transfer> {
    return this.client
      .get<Transfer>(`/inpayments/${referenceId}`)
